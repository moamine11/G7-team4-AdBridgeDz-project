...
router.get('/', async (req, res) => {
  try {
    const {
      agencyId,
      category: categoryId,    // Utilizing categoryId to improve code readability and maintain consistent naming conventions
      q,
      minPrice,
      maxPrice,
      location,
...
router.post('/:id/book', authMiddleware, async (req, res) => {
  try {
    ...
    const booking = new Booking({
      ...
      post: postId,
      agency: post.agency._id,  // Reference to the specific Advertising Agency that originally published this advertisement post
      requestDescription,
      status: 'Pending',
    });
    await booking.save();
